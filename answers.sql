...
SELECT m.name, b.name, b.headquarters FROM models AS m JOIN brands AS b USING (brand_id) WHERE m.name = 'Mustang';
...
SELECT * FROM models WHERE model_id >= 15 AND model_id < 25 ORDER BY year ASC; 
...
SELECT  m.year, m.name, m.model_id, a.name FROM models AS m LEFT JOIN awards AS a ON (a.winner_id = m.model_id) WHERE m.year >= 1964;
...
SELECT b.name, m.year, m.name FROM models AS m LEFT JOIN brands AS b ON b.brand_id = m.brand_id ORDER BY b.name;
...
SELECT b.name, m.name, m.year, b.discontinued, (b.discontinued - m.year) as years_until_brand_discontinued FROM models AS m 
LEFT JOIN brands as b ON m.brand_id = b.brand_id WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
...
SELECT b.name, b.founded FROM brands AS b FULL JOIN models AS m ON m.brand_id = b.brand_id WHERE m.model_id IS NULL;
...
SELECT b.brand_id FROM brands AS b FULL JOIN models AS m ON m.brand_id = b.brand_id GROUP BY b.brand_id HAVING COUNT(m.model_id) > 5;
